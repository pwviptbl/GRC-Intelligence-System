    ...
    <aside class="sidebar">
      ...
      <nav>
        ...
        <div class="nav-submenu-group" x-show="menuAtivosAberto" style="display: none;" x-transition>
          <a href="{{ route('clientes.index') }}" class="nav-btn submenu" :class="{ 'active': view.includes('clientes') }">
            <span class="icon" style="opacity: 0.8;">🏢</span> Clientes
          </a>
          <a href="{{ route('softwares.index') }}" class="nav-btn submenu" :class="{ 'active': view.includes('softwares') }">
            <span class="icon" style="opacity: 0.8;">💾</span> Softwares
          </a>
          <a href="{{ route('instancias.index') }}" class="nav-btn submenu" :class="{ 'active': view.includes('instancias') }">
        ...
        <div class="nav-submenu-group" x-show="menuGovernancaAberto" style="display: none;" x-transition>
          <a href="{{ route('politicas.index') }}" class="nav-btn submenu" :class="{ 'active': view.includes('politicas') }">
            <span class="icon" style="opacity: 0.8;">📄</span> Políticas
          </a>
          <a href="{{ route('procedimentos.index') }}" class="nav-btn submenu" :class="{ 'active': view.includes('procedimentos') }">
        ...
        <a href="{{ route('treinamentos.index') }}" class="nav-btn" :class="{ 'active': view.includes('treinamentos') }">
      ...
      <div class="sidebar-footer">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" style="margin-top:10px;background:rgba(255,83,112,.1);border:1px solid rgba(255,83,112,.2);color:var(--red);border-radius:6px;padding:6px 12px;font-size:11px;cursor:pointer;width:100%">
              🚪 Sair
            </button>
        </form>
      </div>
